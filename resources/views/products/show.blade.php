            ...
            <nav class="flex mb-8 bg-white rounded-xl shadow-md px-6 py-4 border border-gray-100" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-3 text-sm">
                    <li><a href="{{ route('home') }}"
                            class="text-gray-500 hover:text-primary-600 transition-colors font-medium flex items-center">
                    ...
                    <li><a href="{{ route('products.index') }}"
                            class="text-gray-500 hover:text-primary-600 transition-colors font-medium">Products</a></li>
                    <li><span class="text-gray-400">/</span></li>
                ...
                <div class="space-y-6" x-data="productStockMonitor({{ $product->id }}, {{ $product->stock }}, {{ $product->low_stock_threshold }})">
                    <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                        <div class="mb-6">
                            ...
                            <p class="text-sm text-gray-600 bg-gray-100 px-4 py-2 rounded-lg inline-block">SKU: <span
                                    class="font-semibold">{{ $product->sku }}</span></p>
                        </div>

                        <!-- Price -->
                        <div class="flex items-center space-x-4 mb-6 pb-6 border-b border-gray-200">
                            <span
                                class="text-5xl font-bold bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent">${{ number_format($product->price, 2) }}</span>
                            @if ($product->compare_price && $product->compare_price > $product->price)
                                <div class="flex flex-col">
                                    <span
                                        class="text-xl text-gray-500 line-through">${{ number_format($product->compare_price, 2) }}</span>
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg">
                        ...
                        @if ($product->average_rating > 0)
                            <div class="flex items-center space-x-3 mb-6 pb-6 border-b border-gray-200">
                                <div
                                    class="flex items-center bg-gradient-to-r from-yellow-400 to-yellow-500 px-4 py-2 rounded-xl shadow-lg">
                                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        <div class="space-y-4 mb-6 pb-6 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                ...
                                <span x-show="currentStock > 0 && !isLowStock"
                                    class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                ...
                                <span x-show="currentStock > 0 && isLowStock"
                                    class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                ...
                                <span x-show="currentStock <= 0"
                                    class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                ...
                                <div class="bg-gray-50 rounded-xl p-4">
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-4 shadow-inner">
                                        <div class="h-4 rounded-full transition-all shadow-lg"
                                            :class="currentStock > 0 ? (isLowStock ?
                        ...
                        <div class="space-y-3">
                            <button @click="addToCart()" :disabled="currentStock <= 0"
                                ...
                                :class="currentStock <= 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' :
                ...
                <div class="border-b border-gray-200 mb-8">
                    <nav class="-mb-px flex space-x-8">
                        <button @click="activeTab = 'description'"
                            :class="activeTab === 'description' ? 'border-primary-600 text-primary-600 bg-primary-50' :
                        ...
                        <button @click="activeTab = 'reviews'"
                            :class="activeTab === 'reviews' ? 'border-primary-600 text-primary-600 bg-primary-50' :
                                'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                            class="border-b-4 py-4 px-6 text-lg font-semibold transition-all rounded-t-xl">
                            <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            Reviews
                            <span
                                class="ml-1 px-2 py-1 bg-primary-600 text-white rounded-full text-sm">{{ $product->reviews->count() }}</span>
                        </button>
                    </nav>
                </div>
                    ...
                    <div x-show="activeTab === 'reviews'">
                        @if ($product->reviews->count() > 0)
                            <div class="space-y-6">
                                @foreach ($product->reviews as $review)
                                    <div
                                        class="bg-gradient-to-r from-gray-50 to-white rounded-2xl p-6 border border-gray-200 hover:shadow-lg transition-shadow duration-200">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center space-x-3 mb-2">
                                                    <div
                                                        class="w-12 h-12 bg-gradient-to-br from-primary-500 to-primary-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg">
                                                        {{ substr($review->user->name, 0, 1) }}
                                                    </div>
                                                    <div>
                                                        <span
                                                            class="font-bold text-lg text-gray-900">{{ $review->user->name }}</span>
                                                        @if ($review->is_verified)
                                                            <span
                                                                class="ml-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md">
                                                ...
                                                @if ($review->title)
                                                    <h4 class="font-bold text-lg text-gray-900 mb-2">{{ $review->title }}
                                                    </h4>
                                                @endif
                                                <p class="text-gray-700 leading-relaxed">{{ $review->comment }}</p>
                                            </div>
                                            <span
                                                class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-lg">{{ $review->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <div class="text-center py-16">
                                ...
                                <p class="text-gray-500 text-lg font-medium">No reviews yet. Be the first to review this
                                    product!</p>
                            </div>
                        @endif
                    </div>
